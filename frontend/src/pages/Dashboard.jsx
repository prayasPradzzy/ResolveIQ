    ...
    <section className="space-y-8">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/50 backdrop-blur pb-4 border-b border-slate-200/50">
        <div>
          ...
          <h2 className="text-3xl lg:text-4xl font-bold tracking-tight text-slate-900">Dashboard Overview</h2>
        </div>
        
        <div className="flex items-center gap-3 self-start sm:self-auto hidden sm:flex">
           <button className="relative p-2 rounded-full hover:bg-slate-200 text-slate-500 transition-colors">
      ...
      <div className="grid gap-4 rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm sm:grid-cols-2 lg:w-2/3 xl:w-1/2">
        <label className="text-sm text-slate-700 block">
          <span className="mb-1.5 block font-semibold text-slate-800">Filter by Severity</span>
          <select
            value={filters.severity}
            onChange={(event) => setFilters((previous) => ({ ...previous, severity: event.target.value }))}
            className="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-700 outline-none focus:bg-white focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all cursor-pointer hover:bg-slate-100 shadow-sm"
          >
        ...
        <label className="text-sm text-slate-700 block">
          <span className="mb-1.5 block font-semibold text-slate-800">Filter by Category</span>
          <select
